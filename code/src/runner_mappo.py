"""
Training loop for Multi-Agent Proximal Policy Optimisation
...
class Runner():
    ...
    def __init__(self, config):
        ...
        if self.algorithm_name == 'mappo': # Centralised learning
            self.mappo_agent = MAPPO(agents=1, alpha=self.all_args.lr, gamma=self.gamma, eps_clip=self.eps_clip, ppo_epochs=self.ppo_epoch,\
            ...
            self.mappo_agent = MAPPO(agents=self.num_agents, alpha=self.all_args.lr, gamma=self.gamma, eps_clip=self.eps_clip, ppo_epochs=self.ppo_epoch,\
    ...
    def run(self):
        ...
        print(f'Run: code/src/runner_mappo.py')
        ...
        for episode in range(self.first_episode, self.episodes):
            ...
            for step in range(1, self.env.env.numSteps):
                ...
                # Select action with MAPPO
                for n in range(self.num_agents):
                    if switch_idx[n] == switch_indicator:
                        action_gdy_live[n] = action_gdy[n]
                        action_alg[n], action_logprob, state_val = self.mappo_agent.choose_actions(self.agent_idx[n], obs[0,n,:]) 
                ...
                for n in range(self.num_agents):
                    if switch_idx[n] == switch_indicator:
                        self.mappo_agent.remember(self.agent_idx[n], obs[0,n,:], state_val, action_alg[n], rewards[0,n,0], action_logprob)
                obs = _obs
                switch_indicator += 1
                ...
                # Start training:
                if target_cnt % self.cnt_model_update == 0 and total_step > warmup_steps:
                    self.mappo_agent.learn()
                if target_cnt % self.cnt_global_update == 0 and total_step > warmup_steps and self.mappo_agent.use_federated:
                    self.mappo_agent.aggregate_global()
                target_cnt += 1
            ...
            # Checkpoint
            if done_episodes_rewards[-1][0] > prev_reward and total_step > 2*warmup_steps:
                prev_reward = done_episodes_rewards[-1][0]
                self.mappo_agent.save_model()
    ...
    def reset(self):
        ...
        for n in range(self.num_agents):
            action_alg[n] = self.mappo_agent.choose_action(self.agent_idx[n], obs[0,n,:])
