...
from src.trainer.utils.util import get_gard_norm, huber_loss, mse_loss
from src.trainer.utils.valuenorm import ValueNorm
...
class MATTrainer:
    ...
    def ppo_update(self, sample):
        ...
        old_action_log_probs_batch = self._check(old_action_log_probs_batch).to(**self.tpdv)
        adv_targ = self._check(adv_targ).to(**self.tpdv)
        value_preds_batch = self._check(value_preds_batch).to(**self.tpdv)
        return_batch = self._check(return_batch).to(**self.tpdv)
        active_masks_batch = self._check(active_masks_batch).to(**self.tpdv)
